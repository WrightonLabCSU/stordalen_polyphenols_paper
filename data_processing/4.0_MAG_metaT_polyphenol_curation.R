...
mag_geTMM = read_excel("Supplementary_Data_3.xlsx",sheet="metaT_MAGs")
# read in gtdb v2
...
phylum=read_excel("Supplementary_Data_3.xlsx",sheet="color_codes")

# read in MAG metaG abundance
abund = read_excel("Supplementary_Data_3.xlsx",sheet="metaG_MAGs")
...
data1 = left_join(ave_tidy_abund,mag_encoded_paths,by=c("MAG"="mag"),relationship = "many-to-many")
#add metaT 
data1$path=tolower(data1$path)
tidy_metaT_paths$Transformation=tolower(tidy_metaT_paths$Transformation)
data2 = left_join(data1,tidy_metaT_paths,by=c("MAG","site","depth","path"="Transformation"),relationship = "many-to-many")
...
ave_paths1[is.na(ave_paths1)] <- 0
ave_paths1=as.data.frame(ave_paths1)
row.names(ave_paths1)=ave_paths1[,1]
ave_paths1=ave_paths1[,-1]

pheatmap(t(ave_paths1),cluster_rows = FALSE,cluster_cols=TRUE,scale="column")


paths2 = mag_metaT_paths %>%
  ungroup() %>%
  group_by(Transformation,Substrate,oxygen,sample) %>%
  summarise(sum=sum(geTMM)) %>%
   ungroup()%>%
...
ave_paths2[is.na(ave_paths2)] <- 0
ave_paths2=as.data.frame(ave_paths2)
row.names(ave_paths2)=ave_paths2[,1]
ave_paths2=ave_paths2[,-1]

pheatmap(t(ave_paths2),cluster_rows = FALSE,cluster_cols=TRUE,scale="column")


# write table 
write_xlsx(paths,"pathway_table_relabun.xlsx")
# using Supplementary_Data_2 tab polyphenol_genes, figure_2_rxn_number column, remove duplicate entries
# this will now be Supplementary_Data_2 tab "metaT_pathway_relabun"
...
mag_metaT_paths%>%left_join(.,tax,by="MAG")%>%separate(GTDB_v2_r207,into=c("d","p","c","o","f","g","s"),sep=";")%>%mutate(genus=paste(d,p,c,o,f,g,sep=";"))%>%filter(geTMM>0)%>%select(genus,site,pathway)%>%filter()
...
category=read_excel("../../../project_data/metaT/mapped_97perMAGs/metaT_adj_camper.xlsx",sheet="categories")
